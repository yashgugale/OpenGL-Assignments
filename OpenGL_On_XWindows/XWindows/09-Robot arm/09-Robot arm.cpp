...
void CreateWindow(void)
{
	...
	XStoreName(gpDisplay, gWindow, "Robot Arm");
...
void display(void)
{
	...
	glTranslatef(1.0f, 0.0f, 0.0f);	//Save this configuration by pushing new matrix on top and doing further operations on that
	glPushMatrix();
	glScalef(2.0f, 0.5f, 1.0f);	//Scale it accordingly
	...
	quadric = gluNewQuadric();
	glColor3f(0.5f, 0.35f, 0.05f);	//Brown like color
	gluSphere(quadric, 0.5, 10, 10);
	glPopMatrix();
