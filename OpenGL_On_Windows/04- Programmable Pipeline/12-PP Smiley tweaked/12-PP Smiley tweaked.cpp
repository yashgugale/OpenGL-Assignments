...
GLuint gVaoSmileyTweaked;

GLuint gVboSmileyTweakedPosition;
GLuint gVboSmileyTweakedTexture;

//texture variables:
GLuint textureSmileyTweaked;
GLuint texture;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW,
		szClassName,
		TEXT("OpenGL Programmable Pipeline : Smiley Tweaked texture"),
...
void initialize(void)
{
	...
	void uninitialize(void);
	void resize(int, int);
	int LoadGLTextures(GLuint *, TCHAR[]);	//Declare function to load the texture
	void LoadProceduralTexture(void);		// Texture for white color
	...
	*/
	...
	// GLSL version number 4.5 (4.5 * 100 = 450). default = 1.1
	...
	const GLfloat TweakedSmileyVertices[] = 
	{
	...
	// Tweaked Smiley:
	glGenVertexArrays(1, &gVaoSmileyTweaked);
	glBindVertexArray(gVaoSmileyTweaked);

	// A. BUFFER BLOCK FOR VERTICES:
	glGenBuffers(1, &gVboSmileyTweakedPosition);						// Buffer to store vertex position
	glBindBuffer(GL_ARRAY_BUFFER, gVboSmileyTweakedPosition);			// Find that named object in memory
	glBufferData(GL_ARRAY_BUFFER, sizeof(TweakedSmileyVertices), TweakedSmileyVertices, GL_STATIC_DRAW);	// Takes data from CPU to GPU
	...
	glGenBuffers(1, &gVboSmileyTweakedTexture);										// Buffer to store vertex colors
	glBindBuffer(GL_ARRAY_BUFFER, gVboSmileyTweakedTexture);						// Find that named object in memory
	glBufferData(GL_ARRAY_BUFFER, 4 * 2 * sizeof(GL_FLOAT), NULL, GL_DYNAMIC_DRAW);	// Takes data from CPU to GPU
	...
	// Load smiley texture and white texture:
	LoadGLTextures(&textureSmileyTweaked, MAKEINTRESOURCE(IDBITMAP_SMILEY));
...
void display(void)
{
	...
	glBindTexture(GL_TEXTURE_2D, texture);						// 2. bind to the texture
	...
	if (isKeyPressed == 1)
	{
		// BIND with quad Texture (3 steps):
		glActiveTexture(GL_TEXTURE0);								// 1. 0th texture (corresponds to YSG_ATTRIBUTE_TEXTURE0)
		glBindTexture(GL_TEXTURE_2D, textureSmileyTweaked);			// 2. bind to the texture
		glUniform1i(gTextureSamplerUniform, 0);						// 3. 0th sampler enabled (as we have only 1 texture sampler in frag shader
...
void uninitialize(void)
{
	...
	if (gVaoSmileyTweaked)
	{
		glDeleteVertexArrays(1, &gVaoSmileyTweaked);
		gVaoSmileyTweaked = 0;
	}

	// Destroy vbo for Smiley Position:
	if (gVboSmileyTweakedPosition)
	{
		glDeleteBuffers(1, &gVboSmileyTweakedPosition);
		gVboSmileyTweakedPosition = 0;
	}

	// Destroy vbo for Smiley Texture:
	if (gVboSmileyTweakedTexture)
	{
		glDeleteBuffers(1, &gVboSmileyTweakedTexture);
		gVboSmileyTweakedTexture = 0;
	}
