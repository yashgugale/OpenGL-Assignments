...
GLuint gVboSmileyPosition;
GLuint gVboSmileyTexture;

GLuint gMVPUniform;
GLuint gTextureSamplerUniform;	// to catch uniform of sampler

GLuint gTextureSmiley;			// Smiley texture object
...
void initialize(void)
{
	...
	*/
	...
	// Square smiley:
	glGenVertexArrays(1, &gVaoSmiley);
	glBindVertexArray(gVaoSmiley);

	// A. BUFFER BLOCK FOR VERTICES:
	glGenBuffers(1, &gVboSmileyPosition);						// Buffer to store vertex position
	glBindBuffer(GL_ARRAY_BUFFER, gVboSmileyPosition);			// Find that named object in memory
	glBufferData(GL_ARRAY_BUFFER, sizeof(smileyVertices), smileyVertices, GL_STATIC_DRAW);	// Takes data from CPU to GPU
	...
	glGenBuffers(1, &gVboSmileyTexture);					// Buffer to store vertex colors
	glBindBuffer(GL_ARRAY_BUFFER, gVboSmileyTexture);		// Find that named object in memory
	glBufferData(GL_ARRAY_BUFFER, sizeof(smileyTexCoords), smileyTexCoords, GL_STATIC_DRAW);	// Takes data from CPU to GPU
...
void display(void)
{
	...
	glBindTexture(GL_TEXTURE_2D, gTextureSmiley);	// 2. bind to the texture
...
void uninitialize(void)
{
	...
	if (gVaoSmiley)
	{
		glDeleteVertexArrays(1, &gVaoSmiley);
		gVaoSmiley = 0;
	}


	// Destroy vbo for Smiley Position:
	if (gVboSmileyPosition)
	{
		glDeleteBuffers(1, &gVboSmileyPosition);
		gVboSmileyPosition = 0;
	}

	// Destroy vbo for Smiley Texture:
	if (gVboSmileyTexture)
	{
		glDeleteBuffers(1, &gVboSmileyTexture);
		gVboSmileyTexture = 0;
